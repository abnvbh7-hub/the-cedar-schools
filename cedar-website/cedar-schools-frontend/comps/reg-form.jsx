...
export default function RegForm({onClose}) {
  ...
  const [submitedd,setsubmitted] = useState(false)
 ...
 async function submitreq(){
   try{
     const res = await fetch("https://8bb675c2-0c6e-40f4-aeb8-e4f75f4e3e0c-00-3b39ytgxjf42c.pike.replit.dev/register",{
        "method":"POST",
...
async function submitForm(){
  ...
  return (
    <>
    {!submitedd ? (<div className="rf-overlay">
      <div className="rf-mcon">
        <div className="rf-title-con">
          <img className="rf-logo" src="/logo.png" />
          <div className="rf-title">ENQUIRE US</div>
          <div className="rf-close" onClick={onClose} ><img  className="rf-close-logo" src="./close.png"></img></div>
        </div>
        ...
        <div className="rf-submit-con">
          <div className="rf-submit" onClick={()=>submitForm()}>SUBMIT</div>
        </div>
      </div>
    </div>) : <div className="emp"></div>}
      </>
  );
}
